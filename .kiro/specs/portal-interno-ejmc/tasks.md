# Tarefas de Implementação — Portal Interno EJMC
...
- [~] 8.5 Implementar barra de progresso visual para cada meta
- [~] 8.6 Implementar indicador visual de meta vencida (prazo passado + progresso < 100%)
- [~] 8.7 Implementar formulário de criação de meta (Diretor/Admin apenas)
- [~] 8.8 Implementar atualização de progresso (Diretor/Admin apenas)

### Task 9: Módulo KPIs por Área

- [~] 9.1 Criar API Routes: `GET /api/kpis`, `POST /api/kpis/:id/values`, `POST /api/kpis` (admin configura novos KPIs)
- [~] 9.2 Implementar validação: valor numérico com até 2 casas decimais, formato e intervalo por indicador
- [~] 9.3 Criar página `/kpis` com seletor de área e listagem de indicadores
- [~] 9.4 Implementar exibição do valor mais recente e data de inserção para cada KPI
- [~] 9.5 Implementar formulário de inserção de valor de KPI
- [~] 9.6 Implementar configuração de KPIs adicionais por área (Admin): nome max 60 chars, unidade de medida
- [~] 9.7 Seed dos KPIs pré-definidos: inadimplência, capacidade produtiva, congelamentos, NPS, CSAT
...
### Task 12: Módulo Portfólio de Serviços
...
### Task 16: Módulo Pontuação (Infrações)
...
### Task 17: Módulo Reserva de Computadores
...
### Task 20: Testes de Propriedade (Property-Based Testing)
